...
for_plot %>% 
    ...
    scale_x_log10()+
    scale_y_log10()+
    labs(x = "observed frequencies (log)",
         y = "expected frequencies (log)")
...
# What are the relation between expected and observed values?
...
for_plot %>% 
    ...
    geom_smooth(method = "lm", se = FALSE, linetype = 2)+
    facet_wrap(glottocode~approach, scales = "free")+
    scale_x_log10()+
    scale_y_log10()+
    labs(x = "observed frequencies (log)",
         y = "expected frequencies (log)")

ggsave("for_linearity_relation.png", width = 12, height = 10)
library(lme4)
library(lmerTest)

for_plot %>% 
  mutate(observed_probabilities_log = log(observed_probabilities), 
         expected_probabilities_log = log(expected_probabilities)) %>% 
lmer(data = .,
     formula = expected_probabilities_log ~ observed_probabilities_log  + (1|glottocode) + (1|approach)) ->
  fit
...
Our model predicts relations regardless the language and method:

$$\text{expected_probabilities_log} \approx -9  + text{observed_probabilities_log}$$
I wanted model without intecept but it doesn't convere :/
